...
import { parse as miniparse, print as miniprint } from '@bablr/miniparser';

import { buildPatternInternal } from './internal/regex.js';
import { isString, isObject } from './internal/utils.js';

const parseAst = (code) => miniparse(regex, code, 'RegExpLiteral');
const print = (ast) => miniprint(regex, ast);
...
export class Pattern {
  constructor(expr) {
    ...
    this.source = this.source != null ? this.source : print(ast.value.pattern);
    this.flags = this.flags != null ? this.flags : print(ast.value.flags);

    Object.assign(this, ast.value.flags.value);
  }
}
