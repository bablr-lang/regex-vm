import { Matcher, TreeNode } from '@bablr/agast-vm-helpers/symbols';
...
export class Pattern {
  constructor(expr) {
    ...
    if (expr instanceof Pattern) {
      return expr;
    } else if (expr.type === Matcher) {
      if (expr.value.type !== TreeNode || expr.value.value.name !== Symbol.for('Pattern'))
        throw new Error();
      ast = expr.value;
    } else {
