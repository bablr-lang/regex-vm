import { get, getCooked, list, printType } from '@bablr/agast-helpers/tree';
...
export const getWordCharSetTester = (node) => {
  const { negate } = node.value.open.attributes;
...
export const getCharClassRangeTester = (node, flags) => {
  const min = get('min', node);
  const max = get('max', node);

  if (flags.ignoreCase) {
    return (c) =>
      inRange(
        upperCode(String.fromCharCode(c)),
        upperCode(getCooked(min)),
        upperCode(getCooked(max)),
      );
  } else {
    return (c) => inRange(c, code(getCooked(min)), code(getCooked(max)));
  }
};

export const getCharClassTester = (node, flags) => {
  const { negate } = node.value.open.attributes;
  const testers = [...list('elements', node)].map((el) => getTester(el, flags));
...
export const getTester = (node, flags) => {
  switch (printType(node.value.open.type)) {
