...
import { arrayMap, isArray } from '@bablr/agast-helpers/object';
...
let expression = (matchers) => (next) => {
  let boundMatchers = arrayMap(matchers, (matcher) => matcher(next));
...
let visitAlternatives = (alternatives, state) => {
  ...
  switch (alternatives.length) {
    ...
    default: return expression(arrayMap(alternatives, alt => compose(reset, visitAlternative(alt, state))));
  }
};
