...
import { get, getRoot, list } from '@bablr/agast-helpers/tree';
...
const visitExpression = (alternatives, state, visit) => {
  ...
  let alternatives_ = [...alternatives];
  // prettier-ignore
  switch (alternatives_.length) {
    case 0: return identity;
    case 1: return compose(reset, visit(alternatives_[0]));
    default: return expression(alternatives_.map(alt => compose(reset, visit(alt))));
  }
};
...
const visitors = {
  ...
  Alternative: (node, state, visit) => {
    return [...list('elements', node)].map((el) => visit(el)).reduce(compose, identity);
  },

  Group: (node, state, visit) => {
    const alternatives = list('alternatives', node);
  ...
  Quantifier: (node, state, visit) => {
    const { min, max, greedy } = node.value.open.attributes;

    const element = get('element', node);
...
export const buildPatternInternal = (frag) => {
  if (patterns.has(frag)) {
    return patterns.get(frag);
  }

  const node = getRoot(frag);

  const flags = {
    ...defaultFlags,
    ...get('flags', node)?.value.open.attributes,
  };
