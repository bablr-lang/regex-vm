...
import objectFrom from 'iter-tools-es/methods/object-from';
import { flattenCapture } from './captures.js';
import { visit, isAnchored } from './ast.js';
...
const visitors = {
  ...
  Pattern: (node, state, visit) => {
    ...
    return compose(
      ...
      capture(++state.cIdx, visitExpression(alternatives, state, visit)),
    );
  },

  Character: (node, state, visit) => {
    return character(node, state.flags);
  },

  CharacterClass: (node, state, visit) => {
    return character(node, state.flags);
  },

  CharacterSet: (node, state, visit) => {
    return character(node, state.flags);
  },
...
export const buildPatternInternal = (node) => {
  const flags = {
    ...defaultFlags,
    ...objectFrom(node.properties.flags?.map((flag) => [flag.attributes.kind, true])),
  };
