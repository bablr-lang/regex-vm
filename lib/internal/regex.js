...
// match a character
const character = (type, value, flags) => (next) => {
  const tester = getTester(type, value, flags);
  return {
    type: sym.cont,
    width: 1,
    name: 'character',
...
const visitors = {
  ...
  Character: (node, state, visit, type) => {
    return character(type, node, state.flags);
  },

  CharacterClass: (node, state, visit, type) => {
    return character(type, node, state.flags);
  },

  CharacterSet: (node, state, visit, type) => {
    return character(type, node, state.flags);
  },
...
export const buildPatternInternal = (ast) => {
  ...
  const seq = visit(node.pattern, pState, visitors);
