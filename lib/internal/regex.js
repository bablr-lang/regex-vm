...
const visitExpression = (alternatives, state, visit) => {
  ...
  // prettier-ignore
  switch (btree.getSize(alternatives)) {
    case 0: return identity;
    case 1: return compose(reset, visit(btree.getAt(0, alternatives).node));
    default: return expression([...btree.traverse(alternatives)].map(alt => compose(reset, visit(alt.node))));
  }
};
...
const visitors = {
  ...
  Alternative: (node, state, visit) => {
    ...
    return [...btree.traverse(elements)]
      .map((el) => visit(btree.getAt(-1, el.node)))
  ...
  Quantifier: (node, state, visit) => {
    ...
    return repeat(visit(btree.getAt(-1, element.node)), qIdx, greedy);
  },
};
