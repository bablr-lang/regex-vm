...
const visitExpression = (alternatives, state, visit) => {
  ...
  // prettier-ignore
  switch (btree.getSize(alternatives.node)) {
    case 0: return identity;
    case 1: return compose(reset, visit(btree.getAt(0, alternatives.node).node));
    default: return expression([...btree.traverse(alternatives.node)].map(alt => compose(reset, visit(alt.node))));
  }
};
...
const visitors = {
  ...
  Alternative: (node, state, visit) => {
    const { elements = { node: [] } } = node.properties;

    return [...btree.traverse(elements.node)]
      .map((el) => visit(btree.getAt(-1, el.node).node))
  ...
  Quantifier: (node, state, visit) => {
    ...
    return repeat(visit(btree.getAt(-1, element.node).node), qIdx, greedy);
  },
};
...
export const buildPatternInternal = (frag) => {
  ...
  const flags = {
    ...defaultFlags,
    ...node.properties.flags?.node.attributes,
  };
