...
const visitExpression = (alternatives, state, visit) => {
  ...
  switch (btree.getSum(alternatives)) {
    ...
    case 1: return compose(reset, visit(btree.getAt(0, alternatives)));
    default: return expression([...btree.traverse(alternatives)].map(alt => compose(reset, visit(alt))));
  }
};
...
const visitors = {
  ...
  Alternative: (node, state, visit) => {
    ...
    return [...btree.traverse(elements)].map(visit).reduce(compose, identity);
  },
